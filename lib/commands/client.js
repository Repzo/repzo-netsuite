...
export const addClients = async (commandEvent) => {
  ...
  try {
    ...
    let response, pagination_info, hasMore;
    ...
    try {
      ...
      await commandLog
        .addDetail(
          `${pagination_info?.totalResults} clients changed since ${commandEvent.app.options_formData[bench_time_key] || "ever"}`,
        )
        .commit();
...
const getRepzoClients = async (repzo, failed_docs_report) => {
  try {
    let repzo_clients = [],
      per_page = 1000,
      pagination_info,
